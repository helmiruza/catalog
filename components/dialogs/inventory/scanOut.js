...
class ScanOutInventory extends React.Component {
  state = {
    tab: 1,
    unique_id: ''
  }
  ...
  handleSubmit = async () => {
    ...
    if (order && selectedItem) {
      let url = `/orders/${order.id}/items/${selectedItem.id}/pack`
      const req = await ApiReq.put(url, {unique_ids: [unique_id]})

      if (req.status === 200) {
        this.props.handleNotif({
          variant: 'success',
          message: 'Inventory added to Delivery Order'
        })
        this.handleReset()
      } else {
        this.props.handleNotif({
...
export default withMobileDialog()(withStyles(useStyles)(ScanOutInventory))
